...
class MethodChannelPurchasesFlutter extends PurchasesFlutterPlatform {
    ...
    ..setMethodCallHandler((MethodCall call) async {
      switch (call.method) {
        case "Purchases-PurchaserInfoUpdated":
          _purchaserInfoUpdateListeners.forEach((listener) => listener(PurchaserInfo.fromJson(call.arguments)));
          break;
      }
    });
  ...
  Future<void> setup(String apiKey, {String? appUserId, bool observerMode = false, String? userDefaultsSuiteName}) async {
  ...
  Future<void> setFinishTransactions(bool finishTransactions) async {
    await _channel.invokeMethod('setFinishTransactions', {
  ...
  /// [purchaserInfoUpdateListener] PurchaserInfo update listener.
   void addPurchaserInfoUpdateListener(PurchaserInfoUpdateListener purchaserInfoUpdateListener) async {
  ...
  /// [listenerToRemove] PurchaserInfoListener reference of the listener to remove.
   void removePurchaserInfoUpdateListener(PurchaserInfoUpdateListener listenerToRemove) async {
  ...
  /// PurchaseType.Subs by default. This parameter only has effect in Android.
   Future<List<Product>> getProducts(List<String> productIdentifiers, {PurchaseType type = PurchaseType.subs}) async {
    final List<dynamic> result =
    await _channel.invokeMethod('getProductInfo', {'productIdentifiers': productIdentifiers, 'type': describeEnum(type)});
  ...
  /// PurchaseType.Subs by default. This parameter only has effect in Android.
   Future<PurchaserInfo> purchaseProduct(String productIdentifier, {UpgradeInfo? upgradeInfo, PurchaseType type = PurchaseType.subs}) async {
  ...
  /// [upgradeInfo] Android only. Optional UpgradeInfo you wish to upgrade from
  /// containing the oldSKU and the optional prorationMode.
   Future<PurchaserInfo> purchasePackage(Package packageToPurchase, {UpgradeInfo? upgradeInfo}) async {
    final prorationMode = upgradeInfo?.prorationMode;
  ...
  /// using [getPaymentDiscount].
   Future<PurchaserInfo> purchaseDiscountedProduct(Product product, PaymentDiscount discount) async {
    final response = await _channel
        .invokeMethod('purchaseProduct', {'productIdentifier': product.identifier, 'signedDiscountTimestamp': discount.timestamp.toString()});
    return PurchaserInfo.fromJson(response["purchaserInfo"]);
  }
  ...
  /// [paymentDiscount] Discount to apply to the product. Retrieve this discount
  /// using [getPaymentDiscount].
   Future<PurchaserInfo> purchaseDiscountedPackage(Package packageToPurchase, PaymentDiscount discount) async {
    final response = await _channel.invokeMethod('purchasePackage', {
      'packageIdentifier': packageToPurchase.identifier,
      'offeringIdentifier': packageToPurchase.offeringIdentifier,
  ...
  /// was a problem restoring transactions.
   Future<PurchaserInfo> restoreTransactions() async {
  ...
  /// [newAppUserID] The appUserID that should be linked to the currently user
   Future<LogInResult> logIn(String appUserID) async {
    Map<dynamic, dynamic> result = await _channel.invokeMethod('logIn', {'appUserID': appUserID});
  ...
  /// current user is anonymous.
   Future<PurchaserInfo> logOut() async {
  ...
  /// Enables/Disables debugs logs
   Future<void> setDebugLogsEnabled(bool enabled) async {
    return await _channel.invokeMethod('setDebugLogsEnabled', {'enabled': enabled});
  }
  ...
  ///
   Future<void> setSimulatesAskToBuyInSandbox(bool enabled) async {
    return await _channel.invokeMethod('setSimulatesAskToBuyInSandbox', {'enabled': enabled});
  }
  ...
  ///
   Future<void> setProxyURL(String url) async {
    return await _channel.invokeMethod('setProxyURLString', {'proxyURLString': url});
  }

  /// Gets current purchaser info, which will normally be cached.
   Future<PurchaserInfo> getPurchaserInfo() async {
    Map<dynamic, dynamic> result = await _channel.invokeMethod('getPurchaserInfo');
    return PurchaserInfo.fromJson(result);
  }
  ...
  /// default
   Future<void> setAutomaticAppleSearchAdsAttributionCollection(bool enabled) async {
    return await _channel.invokeMethod('setAutomaticAppleSearchAdsAttributionCollection', {'enabled': enabled});
  }

  /// If the `appUserID` has been generated by RevenueCat
   Future<bool> get isAnonymous async {
  ...
  /// [productIdentifiers] Array of product identifiers
   Future<Map<String, IntroEligibility>> checkTrialOrIntroductoryPriceEligibility(List<String> productIdentifiers) async {
    Map<dynamic, dynamic> eligibilityMap =
    await _channel.invokeMethod('checkTrialOrIntroductoryPriceEligibility', {'productIdentifiers': productIdentifiers});
    return eligibilityMap.map((key, value) => MapEntry(key as String, IntroEligibility.fromJson(value)));
  }
  ...
  /// promotional subscription is granted through the RevenueCat dashboard.
   Future<void> invalidatePurchaserInfoCache() async {
  ...
  /// [fbAnonymousID] Empty String or null will delete the subscriber attribute.
   Future<void> setFBAnonymousID(String fbAnonymousID) async {
    await _channel.invokeMethod('setFBAnonymousID', {'fbAnonymousID': fbAnonymousID});
  }
  ...
  /// [mparticleID] Empty String or null will delete the subscriber attribute.
   Future<void> setMparticleID(String mparticleID) async {
  ...
  /// [airshipChannelID] Empty String or null will delete the subscriber attribute.
   Future<void> setAirshipChannelID(String airshipChannelID) async {
    await _channel.invokeMethod('setAirshipChannelID', {'airshipChannelID': airshipChannelID});
  }
  ...
  /// By default, is an empty list and no feature support will be checked.
   Future<bool> canMakePayments([List<BillingFeature> features = const []]) async {
    return await _channel.invokeMethod('canMakePayments', {'features': features.map((e) => e.index).toList()});
  }
  ...
  /// [discount] The `Discount` to apply to the product.
   Future<PaymentDiscount> getPaymentDiscount(Product product, Discount discount) async {
    Map<dynamic, dynamic> result =
    await _channel.invokeMethod('getPaymentDiscount', {'productIdentifier': product.identifier, 'discountIdentifier': discount.identifier});
